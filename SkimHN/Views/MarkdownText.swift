...
struct MarkdownText: View {
    ...
    private func topSpacing(at index: Int, in blocks: [Block]) -> CGFloat {
        ...
        case (.bullet, .bullet),
             (.numbered, .numbered):
            return 4   // consecutive list items
        case (.heading, _):
            return 6   // first block under a heading
        case (_, .heading):
            return 16  // generous break before a new section
        case (.bullet, .paragraph),
             (.numbered, .paragraph):
            return 14  // list → new paragraph reads as a new section
        default:
            return 10  // paragraph → paragraph and the rest
        }
    }
