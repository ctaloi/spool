...
struct StoryDetailView: View {
        ...
        .toolbar {
            ToolbarItemGroup(placement: .topBarTrailing) {
                Button {
                    ...
                    // `headphones.badge.plus` doesn't exist in the SF
                    // Symbols catalog (renders blank). Composing it
                    // manually: base `headphones` glyph with a small
                    // `plus.circle.fill` overlay in the corner when
                    // the story isn't queued. Once queued, the glyph
                    // swaps to a checkmark with a clean symbol
                    // replace transition.
                    Image(systemName: isQueued ? "checkmark.circle.fill" : "headphones")
                        ...
                        .overlay(alignment: .bottomTrailing) {
                            if !isQueued {
                                Image(systemName: "plus.circle.fill")
                                    ...
                                    .foregroundStyle(Color(.systemBackground), Theme.accent)
                                    .offset(x: 6, y: 4)
                                    .transition(.opacity)
                            }
                        }
                }
                .accessibilityLabel(isQueued ? "Remove from Spool" : "Add to Spool")
    ...
    private var isQueued: Bool {
        spool.contains { $0.id == viewModel.story.id }
    }
    ...
    private func sheetDestination(for sheet: DetailSheet) -> some View {
        ...
        case .storyReply:
            // Root-level comment on the story itself. HN treats this
            // identically to a comment reply server-side — same
            // `/comment` endpoint, the story id just becomes the
            // parent. The snippet is the story title so the user can
            // see what they're commenting on while composing.
            ReplyView(
                parentID: viewModel.story.id,
                parentAuthor: viewModel.story.by,
                parentSnippet: viewModel.story.title ?? ""
            )
            .environmentObject(auth)
    ...
    private func loadHeroImage() async {
        ...
        defer {
            ...
            withAnimation(.easeInOut(duration: Theme.AnimationDuration.medium)) {
                heroLookupComplete = true
            }
        }
        do {
    ...
    /// Flat meta line shown next to the vote button on the detail
    /// header: author · time. Score lives inside the vote button.
    /// Comment count moved to the comments-section header below —
    ...
    private var commentsSection: some View {
        HStack(spacing: 8) {
            // Just the count + bubble icon — the word "Comments" was
            // wrapping the header on small phones where the two
            // action buttons (Comment / Ask) ate the remaining width.
            // The icon already conveys "comments"; the count alone is
            // unambiguous and locks to a single line under any width.
            Label("\(viewModel.story.descendants ?? 0)",
                  systemImage: "bubble.left.and.bubble.right.fill")
                .font(Theme.Typography.sectionTitle)
                .foregroundStyle(.secondary)
                .lineLimit(1)
                .fixedSize()
                .accessibilityLabel("\(viewModel.story.descendants ?? 0) comments")
            Spacer()
            // Top-level reply on the story itself. Pops the same
            // ReplyView the per-comment Reply button uses, but with
            // the story id as parent. Routes to login first if the
            // user isn't signed in — same pattern as the per-comment
            // button so we never present a dead "Post" button.
            Button {
                if auth.isLoggedIn {
                    presentedSheet = .storyReply
                } else {
                    presentedSheet = .login
                }
            } label: {
            ...
            .controlSize(.small)
            .tint(Theme.accent)
            .accessibilityLabel("Comment on this story")
            ...
            LazyVStack(alignment: .leading, spacing: 0) {
                ForEach(viewModel.visibleComments) { node in
                    ...
                    CommentView(
                        ...
                        onToggle: {
                            withAnimation(.easeInOut(duration: Theme.AnimationDuration.micro)) {
...
enum DetailSheet: Identifiable {
    ...
    var id: String {
        ...
        case .storyReply:          return "storyReply"
